...
class AnomalyDetector:
    ...
    def detect(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        results = data.copy()
        results['anomaly_score'] = -self.model.score_samples(data)  # Convert to positive scores
    ...
    def get_top_drivers(self,
                       ...
                       n_features: int = 2,
                       max_samples: int = 10) -> dict[int, list[tuple[str, float]]]:
        """
        ...
        """
        # Get only the anomaly samples
        anomaly_data = anomalies[anomalies['is_anomaly']].drop(['anomaly_score', 'is_anomaly'], axis=1)

        # Limit the number of samples to analyze
        if len(anomaly_data) > max_samples:
            anomaly_data = anomaly_data.iloc[:max_samples]
            logger.info(f"Analyzing first {max_samples} anomalies out of {len(anomalies[anomalies['is_anomaly']])}")

        # Get SHAP values
        explainer = shap.TreeExplainer(self.model)
        shap_values = explainer.shap_values(anomaly_data)

        # Get top features for each anomaly
        top_drivers = {}
        for idx, row_idx in enumerate(tqdm(anomaly_data.index, desc="Analyzing anomalies")):
            feature_importance = list(zip(self.feature_names,
                                       np.abs(shap_values[idx]), strict=False))
    ...
    def plot_anomalies(self,
                      anomalies: pd.DataFrame,
                      drivers: dict[int, list[tuple[str, float]]]) -> None:
        """
        ...
        # Plot top drivers for each anomaly
